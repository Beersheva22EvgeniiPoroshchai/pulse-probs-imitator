...
public class PulseProbesImitatorImpl implements PulseProbesImitator {
	...
	@Value("${app.probes.imitator.jump.increase.prob}")
	int increaseProbability;
	...
	private int getAndUpdateValue(long patientId) {
		...
		int newValue = oldValue == 0 ? (int) getRandomLong(minValue, maxValue) :
	...
	private int getNewValue(int oldValue) {
		boolean isJump = chance(jumpProbability);
	...
	private int getValueDelta(int oldValue, double delta) {
		boolean isIncrease = chance(increaseProbability);
		if (!isIncrease) {
			delta = -delta;
		}
	...
	private boolean chance(int probability) {
		
		return getRandomLong(0, 100) < probability;
	}
